    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background">
      <div className="relative flex w-full items-center justify-between px-4 py-3 md:px-6">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {discordUrl && (
            <a href={discordUrl} target="_blank" rel="noopener noreferrer" className="hidden sm:inline-flex">
          ...
          {showGithub && <span className="hidden sm:inline-flex"><GitHubButton url={githubUrl} /></span>}
          {showDeploy && (
            <a href={`https://vercel.com/new/clone?repository-url=${encodeURIComponent(githubUrl)}`} target="_blank" rel="noopener noreferrer" className="hidden sm:inline-flex">
